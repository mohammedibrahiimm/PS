// https://atcoder.jp/contests/abc167/tasks/abc167_c?lang=en
// C - Skill Up
#include<bits/stdc++.h>
...
int solve(vector<pair<int,vector<int>>>&v,int i,int x,int sum,vector<int>cnt){
	if(i>=v.size()){
		for(auto&it:cnt){
			if(it<x)
				return INT_MAX;
	...
	int mx=INT_MAX;
	mx=min(mx,solve(v,i+1,x,sum,cnt));
	sum+=v[i].first;
...
//	cin >> t;
	while (t--) {
		...
		for(int i=0;i<n;i++){
			cin>>v[i].first;
			v[i].second=vector<int>(m);
			for(auto&it:v[i].second)
				cin>>it;
		}
		vector<int>cnt(m,0);
		int ans=solve(v,0,x,0,cnt);
