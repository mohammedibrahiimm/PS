// https://www.spoj.com/problems/EELCS/en/
...
void solve(int indx,string s,string tmp){
	if(indx>=s.size()){
		if(!tmp.empty())
			sub1.insert(tmp);
		return;
	}
	solve(indx+1,s,tmp+s[indx]);
	solve(indx+1,s,tmp);
}
void solve1(int indx,string s,string tmp){
	if(indx>=s.size()){
		if(!tmp.empty())
			sub2.insert(tmp);
		return;
	}
	solve1(indx+1,s,tmp+s[indx]);
	solve1(indx+1,s,tmp);
}
int main(){
    fast
