// https://vjudge.net/contest/591107#problem/N
// N - Knapsack 
#include<bits/stdc++.h>
...
int knap(int i,int sum,int w,vector<pair<int,int>>&v){
	if(i>=v.size()){
		return (w>=0?sum:-1);
	}
	int mx=INT_MIN;
	mx=max(mx,knap(i+1,sum+v[i].second,w-v[i].first,v));
	mx=max(mx,knap(i+1,sum,w,v));
	return mx;
}
int main(){
    fast
