...
// C. Xor-tree	Codeforces Round 245 (Div. 2) 
// brute force data structures dfs and similar trees *1300
...
//	cin >> t;
	while (t--) {
		...
		for(int i=1;i<=n;i++){cin>>v1[i].first;}
		for(int i=1;i<=n;i++){cin>>v1[i].second;}
		...
		while(!q.empty()){
			...
			for(auto&it:v[nw]){
				if(par[it]==-1){
					...
					q.push(it);
					cnt[it]=cnt[par[nw]];
					if(v1[it].first!=v1[it].second&&cnt[par[nw]]%2==0){
						ans.push_back(it);
						cnt[it]++;
					}
					else if(v1[it].first==v1[it].second&&cnt[par[nw]]%2){
						ans.push_back(it);
						cnt[it]++;
					}
				}
