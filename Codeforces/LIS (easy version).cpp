// https://vjudge.net/contest/591107#problem/Q
// Q - LIS (easy version)
#include<bits/stdc++.h>
...
int lis(int indx,vector<int>&v,int cnt,int lst){
	if(indx>=v.size()){return cnt;}
	int mx=0;
	if(v[indx]>lst)
		mx=max(mx,lis(indx+1,v,cnt+1,v[indx]));
	mx=max(mx,lis(indx+1,v,cnt,lst));
	return mx;
}
int main(){
    fast
