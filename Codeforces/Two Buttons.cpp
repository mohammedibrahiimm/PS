...
// B. Two Buttons Codeforces Round 295 (Div. 2)
...
//	cin >> t;
	while (t--) {
		...
		visit[n]=0;
		int cnt=M;
		while(!q.empty()&&cnt&&q.front()!=m){
			int nw=q.front();
//			cout<<nw<<" ";
			q.pop();
			if(nw*2<M&&visit[nw*2]==-1){
				q.push(nw*2);
				visit[nw*2]=visit[nw]+1;
			}
			if(nw-1>0&&visit[nw-1]==-1){
				q.push(nw-1);
				visit[nw-1]=visit[nw]+1;
			}
			cnt--;
		}
		cout<<visit[m]<<" ";
