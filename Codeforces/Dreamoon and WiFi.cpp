...
double cnt=0,cnt1=0;
void solve(int difm,int difp,int indx,string s){
	if(indx==s.size()){
		if(difm==0&&difp==0){cnt++;}
		cnt1++;
		return;
	}
	if(s[indx]=='?'){
		solve(difm-1,difp,indx+1,s);
		solve(difm,difp-1,indx+1,s);
	}
	else
		solve(difm,difp,indx+1,s);
}
int main(){
    fast
//	TxtIO
...
//	cin >> t;
	while (t--) {
		...
		if(m2>m1||p2>p1)
			...
			int difm=(m1-m2),difp=(p1-p2);
			solve(difm,difp,0,s1);
			cout<<fixed<<setprecision(9)<<cnt/cnt1;
		}
	}
}
