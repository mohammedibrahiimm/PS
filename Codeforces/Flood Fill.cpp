// https://vjudge.net/contest/591107#problem/V
// V - Flood Fill  
#include<bits/stdc++.h>
...
int cnt=0;
void connect(int x,int y,vector<vector<char>>&v){
	if(x<0||x>=v.size()||y<0||y>=v[0].size()||vis[x][y]){return;}
	vis[x][y]=1;
	cnt++;
	for(int i=0;i<4;i++){
		int nwx=x+dx[i];
		int nwy=y+dy[i];
		if(nwx>=0&&nwx<v.size()&&nwy>=0&&nwy<v[0].size()&&v[nwx][nwy]==v[x][y]){
			connect(nwx,nwy,v);
		}
	}
}
